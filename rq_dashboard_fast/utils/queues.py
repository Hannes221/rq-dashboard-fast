...
def get_queues(redis_url: str) -> list[Queue]:
    try:
        ...
        raise HTTPException(status_code=500, detail=str("Error reading Queues for redis connection: ", error))
...
def get_job_registry_amount(redis_url: str) -> list[QueueRegistryStats]:
    try:
        ...
        raise HTTPException(status_code=500, detail=str("Error reading registrys for queue: ", error))
...
def delete_jobs_for_queue(queue_name, redis_url) -> list[str]:
    try:
        ...
        raise HTTPException(status_code=500, detail=str("Error deleting jobs in queue: ", error))